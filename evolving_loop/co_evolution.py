...
class PolicyEvaluation:
    ...
    def mean_smae(self) -> float:
        return self.diagnostics["mean_smae"]

    @property
    def mean_srmse(self) -> float:
        return self.diagnostics["mean_srmse"]
...
def evaluation_diagnostics(
    ...
    return {
        "mean_smae": statistics.fmean(item.final_smae for item in outcomes),
        "mean_srmse": statistics.fmean(item.final_srmse for item in outcomes),
        ...
        "mean_contextual_oracle_smae": statistics.fmean(
            item.contextual_oracle_smae for item in outcomes
        ),
        "mean_contextual_oracle_srmse": statistics.fmean(
            item.contextual_oracle_srmse for item in outcomes
        ),
...
def forecast_utility(outcomes: Sequence[ResolvedOutcome]) -> float:
    ...
    return -statistics.fmean(item.final_srmse for item in outcomes)
