...
def _summarize(mode: str, scores: list[dict], library: SkillLibrary | None, results_path: Path) -> dict:
    ...
    return {
        ...
        # Backward-compatible report keys for consumers written before the
        # Dr-CiK metric migration.  They are aliases only and are not used for
        # current model selection.
        "mean_smape": _round3(_mean([s["smae"] for s in scores])),
        "mean_smape_first_half": _round3(_mean([s["smae"] for s in first_half])),
        "mean_smape_second_half": _round3(_mean([s["smae"] for s in second_half])),
