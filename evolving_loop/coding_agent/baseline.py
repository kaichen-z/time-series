...
from common.metrics import scaled_mae, scaled_rmse
    ...
    with open(results_path, "w", encoding="utf-8") as results_file:
        ...
        for task in tasks:
            ...
            if library is not None and result.action != "fallback":
                library.record_use(result.skill_name, score["smae"])
...
def _summarize(mode: str, scores: list[dict], library: SkillLibrary | None, results_path: Path) -> dict:
    """First-half vs second-half mean sMAE and sRMSE: did it get better as the library grew."""
    half = len(scores) // 2
