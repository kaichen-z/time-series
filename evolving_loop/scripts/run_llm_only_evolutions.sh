...
EA_CODEX_CACHE_DIR="${EA_CODEX_CACHE_DIR:-}"
EA_LIMIT="${EA_LIMIT:-}"
...
if [[ "$EA_EVOLVE_TARGET" != "auto" && "$EA_EVOLVE_TARGET" != "coding" && "$EA_EVOLVE_TARGET" != "retrieval" && "$EA_EVOLVE_TARGET" != "decision" ]]; then
    die "EA_EVOLVE_TARGET must be auto, coding, retrieval, or decision; got '$EA_EVOLVE_TARGET'"
fi
if [[ "$EA_DRY_RUN" != "1" && ! -e "$TASKS_FILE" ]]; then
    die "task path does not exist: $TASKS_FILE"
fi
if [[ "$EA_DRY_RUN" != "1" && -n "$EA_SEED_POLICY_PATH" && ! -f "$EA_SEED_POLICY_PATH" ]]; then
    die "seed policy does not exist: $EA_SEED_POLICY_PATH"
fi
if [[ "$EA_SUCCESSIVE_HALVING" != "0" && "$EA_SUCCESSIVE_HALVING" != "1" ]]; then
    die "EA_SUCCESSIVE_HALVING must be 0 or 1; got '$EA_SUCCESSIVE_HALVING'"
fi
...
for evolution_mode in "${MODES[@]}"; do
    ...
    args=(
        ...
        --codex-cache-dir "$codex_cache_dir"
        --generations "$EA_GENERATIONS"
        --children "$EA_CHILDREN"
    ...
    if [[ "$evolution_mode" == "source" ]]; then
        ...
        if [[ "$EA_SUCCESSIVE_HALVING" == "1" ]]; then
            args+=(
                --successive-halving
        ...
        if [[ -n "$EA_SEED_POLICY_PATH" ]]; then
            args+=(--seed-policy-path "$EA_SEED_POLICY_PATH")
        fi
    fi
    if [[ -n "$EA_LIMIT" ]]; then
        args+=(--limit "$EA_LIMIT")
