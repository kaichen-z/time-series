...
_file_logger = logging.getLogger("common.trace_file")
_console_logger = logging.getLogger("common.trace_console")
...
def emit(event: TraceEvent) -> None:
    ...
    if event.event_type in ("task_start", "task_end", "method_end", "sandbox_failed"):
...
def _summary_line(event: TraceEvent) -> str:
    if event.event_type == "method_end":
        ...
        line = f"[{event.mode}] {event.task_id}: {stage}"
        if event.detail.get("ok"):
            line += f" ok ({event.detail.get('duration_ms')}ms, {event.detail.get('code_chars')} chars)"
        else:
            line += f" FAILED {event.detail.get('error')!r}"
        return line
    if event.event_type == "sandbox_failed":
        return f"[{event.mode}] {event.task_id}: sandbox {event.detail.get('error')!r}"
